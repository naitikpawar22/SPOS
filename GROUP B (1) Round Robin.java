...
class RR {   
    public static void main(String args[]) throws IOException {  
        ...
        for (int i = 0; i < total_time; i++) {  
            ...
            for (int j = 1; j <= n; j++) {  
                if (proc[j][1] != 0) {  
                    proc[j][3]++;  // If process has not completed execution, increment TT by 1  
                    if (j != sel_proc) // If the process has not been currently assigned the CPU, increment WT by 1  
                        proc[j][2]++;  
                } else if (j == sel_proc) {  // Special case: the process has been assigned CPU and has completed execution  
                    proc[j][3]++;  
                }
            }  
            ...
            if (i != 0) {  
                if (sel_proc != time_chart[i - 1]) {  
                    // If CPU has been assigned to a different Process, print the current time and the name of the new Process  
                    System.out.print("--" + i + "--P" + sel_proc);  
                }  
            } else {  
                // If current time is 0, print the name of the First selected Process   
                System.out.print(i + "--P" + sel_proc);  
            }  
            
            if (i == total_time - 1) // Print the time at which execution ends   
